...
final case class Commands(queuesCmd: Option[Seq[QueueCmd]],
...
trait JsonRabbitCmdProtocol extends DefaultJsonProtocol {
  implicit def commandsFormat:RootJsonFormat[Commands] = jsonFormat(Commands, "queues", "exchanges", "bindings")
...
object RabbitCmds extends JsonRabbitCmdProtocol {
  def load():Commands = {
    val cmds = JsonParser(Source.fromFile("./rabbit-commands.json").getLines.mkString).convertTo[Commands]
    // TODO: validation
    cmds
  }
}